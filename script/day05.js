...
console.log(`${typeof(a)}+${typeof(b)}=${typeof(a+b)}`) //N +S = S
console.log('------------------------------------')
//명시적 형변환(문자>숫자, 숫자>문자), 개발자가 직접 데이터형식결정
console.log(`b=${typeof(b)}, b+1=${b+1}`)
//Number() == 형변환 메서드
//괄호 안에 작성한 데이터를 숫자 데이터를 변환한다.
//메서드를 사용할때만 일회성으로 숫자데이터로 인식한다.
...
//동작 이벤트없이 input의 값 확인은 초기값만 인식한다.
//input value 값은 
...
console.log(`userInput type = ${typeof(userInputNumber)}`)
...
//구구단 과제 풀이
console.log('------------------------------------------')
const dan2 = document.querySelector('#dan2')//2단 고정input
